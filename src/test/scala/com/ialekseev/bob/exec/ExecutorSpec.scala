  ...
  "Executor: building" when {
    ...
    "analyzer returns errors" should {
      "return that errors" in {
        ...
        //assert
        result.toEither.right.get should be (LexicalAnalysisFailed(Seq(LexicalError(10, 20), LexicalError(30, 40))).left)
      }
    }
    ...
    "analyzer succeeds & returns constants + scala-code with errors" should {
      "return error with amended offsets" in {
        ...
        Mockito.when(compiler.compile("do()", "import com.ialekseev.bob.dsl._", """var request: HttpRequest = null; var a = "1"; var b = "2"""", """implicit val namespace = Namespace("com", "create"); implicit val description = Description("cool")""")).thenReturn(IoTry.success(CompilationFailed(Seq(CompilationError(400,400,400, "bad!"))).left[String]))

        //act
        val result = executor.build("<scala>do()<end>").run.unsafePerformIO()

        //assert
        result.toEither.right.get should be  (CompilationFailed(Seq(CompilationError(128,128,128, "bad!"))).left)
      }
    }
  ...
  "Executor: running" when {
    ...
    "there are no matching builds" should {
      "return empty" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example2/{$a}/2/"), HttpMethod.GET, Map.empty, Map.empty, none[Body])), ScalaCode("do()")),"super"))
    ...
    "there are no matching builds (incoming POST request doesn't have 'body')" should {
      "return empty" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.POST, Map.empty, Map.empty, some(StringLiteralBody("hello {$name}! You are {$s}, aren't you?")))), ScalaCode("do()")), "abc"))
    ...
    "there is a matching build (with matching 'uri' (without vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(none, HttpMethod.GET, Map.empty, Map.empty, none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/", "GET", Map.empty, Map.empty, none)))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) - even though letter case is different)" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("COM/create/EXAMPLE/", "GET", Map.empty, Map.empty, none)))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'headers' (without vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(none, HttpMethod.GET, Map("header1" -> "secret", "header2" -> "xxx"), Map.empty, none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create", "GET", Map("header1" -> "secret", "header2" -> "xxx", "header3" -> "yyy"), Map.empty, none)))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'headers' (with vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(none, HttpMethod.GET, Map("header1" -> "{$h1}", "header2" -> "app_{$h2}"), Map.empty, none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create", "GET", Map("header1" -> "secret", "header2" -> "app_xxx"), Map.empty, none), "h1" -> "secret", "h2" -> "xxx"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (with vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/{$a}/{$b}/"), HttpMethod.GET, Map.empty, Map.empty, none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/1/2/", "GET", Map.empty, Map.empty, none), "a" -> "1", "b" -> "2"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'queryString' (with vars)) - CASE INSENSITIVE KEYS" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.POST, Map.empty, Map("Q1" -> "start_{$name}_end", "q2" -> "{$surname}", "Q3" -> "super"), none[Body])), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "POST", Map.empty, Map("q1" -> "start_john_end", "Q2" -> "smith", "Q3" -> "super"), none), "name" -> "john", "surname" -> "smith"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (with vars) and 'body (string)' (with vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/{$a}/{$b}/"), HttpMethod.GET, Map.empty, Map.empty, some(StringLiteralBody("hello {$name}! You are {$s}, aren't you?")))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/1/2/", "GET", Map.empty, Map.empty, body), "a" -> "1", "b" -> "2", "name" -> "John", "s" -> "fine"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'body (dictionary)' (with vars))" should {
      "run it" in {
        ...
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(DictionaryBody(Map("hello"-> "{$name}", "state" -> "{$s}, aren't you?"))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body), "name" -> "John", "s" -> "fine"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'body (json)' (without vars))" should {
      "run it" in {
        ...
        val incoming = HttpRequest(some("com/create/example/"), HttpMethod.GET, Map.empty, Map.empty, body)
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(JsonBody(JObject("b" -> JInt(1), "a"-> JString("1")))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body)))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'body (json)' (without vars)) for incoming request with excessive body data" should {
      "run it" in {
        ...
        val incoming = HttpRequest(some("com/create/example/"), HttpMethod.GET, Map.empty, Map.empty, body)
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(JsonBody(JObject("b" -> JInt(1), "a"-> JObject("a2" -> JInt(2))))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body)))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'body (json)' (with vars)) for incoming request with excessive body data" should {
      "run it" in {
        ...
        val incoming = HttpRequest(some("com/create/example/"), HttpMethod.GET, Map.empty, Map.empty, body)
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(JsonBody(JObject("b" -> JString("{$b}"), "a"-> JObject("a2" -> JString("before_{$hel}_after"))))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body), "hel" -> "hello", "b" -> "1"))).thenReturn(IoTry.success("1"))
    ...
    "there is a matching build (with matching 'uri' (without vars) and 'body (json)' (with standalone vars)) for incoming request with Int data for the matching vars" should {
      "run it with all variables as Strings" in {
        ...
        val incoming = HttpRequest(some("com/create/example/"), HttpMethod.GET, Map.empty, Map.empty, body)
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(JsonBody(JObject("b" -> JString("{$b}"), "a"-> JObject("a2" -> JString("{$hel}"))))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body), "hel" -> "2", "b" -> "true"))).thenReturn(IoTry.success("1"))
    ...
    "there is a failed run (compiler throws an exception)" should {
      "return a failure for that run" in {
        ...
        val incoming = HttpRequest(some("com/create/example/"), HttpMethod.GET, Map.empty, Map.empty, body)
        val builds = List(Build(AnalysisResult(Namespace("com", "create"), "cool", Seq.empty, Webhook(HttpRequest(some("example/"), HttpMethod.GET, Map.empty, Map.empty, some(JsonBody(JObject("b" -> JString("{$b}"), "a"-> JObject("a2" -> JString("{$hel}"))))))), ScalaCode("do()")), "abc"))
        Mockito.when(compiler.eval[String]("abc", Seq("request" -> HttpRequestEx("com/create/example/", "GET", Map.empty, Map.empty, body), "hel" -> "2", "b" -> "true"))).thenReturn(IoTry.failure[String](new NullPointerException("bang!")))
