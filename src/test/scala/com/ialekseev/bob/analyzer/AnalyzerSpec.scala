  ...
  "Analyzing" when {
    ...
    "lexer & parser have succeeded with a minimal parse tree" should {
      "map the resulting parse tree to the analysis result" in {
        ...
        //assert
        result should be (AnalysisResult(Namespace("com", "create"), "hello", Map.empty, Webhook("/example", HttpMethod.GET, Map.empty, Map.empty, none), ScalaCode("val a = 1")).right)
      }
    }
    ...
    "lexer & parser have succeeded with a maximum parse tree" should {
      "map the resulting parse tree to the analysis result" in {
        ...
        //assert
        result should be (AnalysisResult(Namespace("com.ialekseev", "create"), "hello", Map("var1" -> "alice", "var2" -> "wonderland"), Webhook("/example", HttpMethod.POST, Map("h1"->"a"), Map("b"->"18"), some(JsonBody(JObject("c"-> JString("19"))))), ScalaCode("val a = 1")).right)
      }
    }
  }
